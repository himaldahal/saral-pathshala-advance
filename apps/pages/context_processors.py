# context_processors.py  — add courses_sidebar to your existing site_settings processor
...
def site_settings(request):
    ...
    if not cached_settings:
        ...
        cache.set('site_settings_cached', cached_settings, 86400)


    # ── Upcoming exams sidebar (1 min cache) ──
    ...
    if not upcoming_exams_sidebar:
        ...
        cache.set('upcoming_exams_sidebar', upcoming_exams_sidebar, 300)

    # ── Courses sidebar (10 min cache) ──
    courses_sidebar = cache.get('courses_sidebar')
    if not courses_sidebar:
        courses_sidebar = list(Course.objects.filter(is_active=True).order_by('name')[:8])
        cache.set('courses_sidebar', courses_sidebar, 600)

    return {
        'site_settings': cached_settings,
        'recaptcha_site_key': getattr(settings, 'RECAPTCHA_SITE_KEY', ''),
        'upcoming_exams_sidebar': upcoming_exams_sidebar,
        'courses_sidebar': courses_sidebar,
    }